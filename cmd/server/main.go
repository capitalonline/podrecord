...
import (
	...
	"github.com/capitalonline/eci-manager/internal/controller"
...
func init() {
	...
	utilruntime.Must(eciv1.AddToScheme(scheme))
...
func main() {
	...
	flag.StringVar(&customerID, "customer-id", "", "The ID of customer.")
	opts := zap.Options{
		Development: true,
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "48087533.eci.cds",
	...
	setupLog.Info(fmt.Sprintf("customer id %v", customerID))
	if customerID == "" {
		setupLog.Error(fmt.Errorf("invalid params"), "customer id is required")
		os.Exit(1)
	}
	...
	if ok := mgr.GetCache().WaitForCacheSync(ctx); !ok {
		setupLog.Error(err, "failed to wait for cache sync")
		os.Exit(1)
	}
	podController := &controller.PodRecordReconciler{
	...
	if err = mgr.GetFieldIndexer().IndexField(ctx, &eciv1.PodRecord{}, constants.FieldSelectorPodName, func(object client.Object) []string {
		...
		return []string{podRecord.Spec.PodName}
	}); err != nil {
		setupLog.Error(err, "failed to index pod records")
		os.Exit(1)
	}
	<-ctx.Done()
	os.Exit(0)
